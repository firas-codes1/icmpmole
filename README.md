...
Upon inspecting 12 packets from tail and 12 from head, it w that ICMP is flagged as bad when coming from same source to same destination repeatedly.
...
The original dataset included Source and Destination ports. However, since ICMP is not on transport layer, there is no need for source and destination ports, only IP addresses, packet length, capture time, and protocol. 
...
2. Encode Protocols to the following values: 1 for ICMP and TCP, and 0 for all others.
3. Encode IP addresses to 1 for internal and trusted IPs (only 192.167.5.22 in the case of the dataset), and 0 for all others.
   The encoding of IPs follows the access control concept of splitting the network to security zones (trusted, untrsuted...etc) which reduces the complexity of the model.
